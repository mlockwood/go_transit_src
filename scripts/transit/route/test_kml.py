...
table = {}
reader = open('{}/reports/gtfs/files/shapes.txt'.format(PATH))
for line in reader:
...
for shape in table:
    ...
    # Write the file
    if not os.path.exists(PATH + '/reports/gtfs/test'):
        os.makedirs(PATH + '/reports/gtfs/test')

    writer = open('{}/reports/gtfs/test/shape_{}.kml'.format(PATH, shape), 'w')
    writer.write('\n'.join(final))
